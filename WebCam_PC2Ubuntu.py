...
if __name__ == '__main__':
    ...
    while (True):
        ...
        filepath = "./data"
        file = os.path.join(filepath, filename1)  # 경로 + 파일 이름 + 확장자 (리눅스 버전)
        file2 = os.path.join(filepath, filename2)  # 경로 + 파일 이름 + 확장자 (리눅스 버전)
        cv2.imwrite(file, frame1)  # 이미지 저장
        cv2.imwrite(file2, frame2)  # 이미지 저장
        print("manage_file : %s 파일을 저장했습니다." % (filename1))

        # ftp 전송
        myfile = open(os.path.join(filepath, filename1), 'rb')  # 파일 경로

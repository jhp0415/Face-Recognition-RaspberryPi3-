...
class FileReceiveHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        while(True):
            ...
            with open(os.path.join(upload_dir, fileName), 'wb') as file:  # 업로드 받을 파일을 생성한다.
                dataMsgId = -1
                prevSeq = 0
...
if __name__ == '__main__':
    ...
    '''
    upload_dir = "C:\\Users\\jh_p0\\Desktop\\network\\raspberryPi\\tcpip\\data"  # 사용자가 데이터를 저장하기 원하는 디렉토리 경로 입력받기
    if os.path.isdir(upload_dir) == False:  # 디렉토리 존재 여부, 없으면?
        os.mkdir(upload_dir)  # 디렉토리 생성

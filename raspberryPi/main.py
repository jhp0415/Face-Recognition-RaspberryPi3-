...
from raspi.frame import Frame                     # 사용자 정의 클래스
#from raspi.video_camera import VideoCamera
from raspi.WebCam import VideoCamera
from raspi.face_detection import FaceDetection
from raspi.manage_file import ManageFile
from raspi.tcpip_client import TCPIPClient

'''
라즈베리파이가 파이카메라를 사용하여 촬영을 시작한다.
촬영 영상에서 사람의 얼굴을 포착하는 경우 해당 프레임을 디렉토리에 저장한다. 
추후 저장된 프레임 영상은 서c버로 전송될 예정이다.
'''
...
if __name__ == '__main__':
    ...
    ob_client = TCPIPClient()
    ob_file = ManageFile(ob_client)            # 프레임을 저장하는 서브 스레드
